...
# Check out the tuning results
tune_results |> show_best('rmse', n = 10)
# Plot the tuning results
tune_results |> autoplot()
```

We could also use `tune_bayes` instead of `tune_grid` for a potentially more efficient tuning process via Bayesian optimization. We can also use our previous tuning results as the "initial" values for the Bayesian optimization process, which can help it find good tuning parameters more quickly.
...
```{r}
#| label: tune-bayes-results

# Check out the tuning results
tune_results_bayes |> show_best('rmse', n = 10)
# Plot the tuning results
tune_results_bayes |> autoplot()
```