...
.b[Problem set] *Classification* soon!

.b[Project 2] Choose your topic.
...
$$
\begin{align}
  \sum_{j=1}^{J} \sum_{i=1}^{n_{j}} \left( y_i - \hat{y}_{R_j} \right)^2
\end{align}
$$
...
Another split: x.sub[2] at 3, which yields .purple[(.b[1]) x.sub[2] < 3] .it[vs.] .pink[(.b[2]) x.sub[2] ≥ 3]
...
Final split: x.sub[2] at 5, which yields .purple[(.b[1]) x.sub[2] < 5] .it[vs.] .pink[(.b[2]) x.sub[2] ≥ 5]
...
.note[Fun tool] Grant McDermott's [`parttree` package](https://grantmcdermott.com/parttree/).
