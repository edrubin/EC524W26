...
.b[Problem sets]
- .it[Cross validation and shrinkage] Due next week.
...
```{r, datatable-default, echo = FALSE, cache = TRUE}
set.seed(1)
...
```{r, clean-default-data, include = FALSE}
# Clean data
...
```{r, boxplot-default-balance, echo = FALSE, cache = TRUE}
...
```{r, plot-default-points, echo = FALSE, cache = TRUE}
# Plot points
...
```{r, plot-default-lpm, echo = FALSE, cache = TRUE}
ggplot(data = default_df, aes(x = balance, y = i_default)) +
geom_point(alpha = 0.05, size = 3.5, color = purple) +
...
```{r, plot-default-logistic, echo = FALSE, cache = TRUE}
ggplot(data = default_df, aes(x = balance, y = i_default)) +
geom_point(alpha = 0.05, size = 3.5, color = purple) +
...
1. takes some *score* (any real number) an maps it to a probability (0 to 1);
1. forces an S-shaped curved through the data (not linear).

.note[Notice:] We just connect probs. $p(X)$ to $\beta_0 + \beta_1 X$ with diff. transforms.

---
## What's .it[logistic]?

.note[Aside:] People also call the .pink.it[logistic function] a .hi[Sigmoid function] (_S-shape_).

This shape works well for modeling .b.purple[natural and social phenomena], _e.g._,

- (constrained) population .b[growth/spread],
- .b[learning] curves and .b[info/tech propagation],
- .b[dose-response] and (crop) .b[yield] curves,
  
and is one of the fundamental elements of .b.orange[neural networks].
<br>
.grey-light.it[(Logisitic regression itself is a very simple neural network.)]
...
.attn[Maximum likelihood estimation] (MLE) searches for the parameters ($\beta_j$'s) that make our data "most likely" given the model we've written down.
...
Let $\sigma(z) = \dfrac{e^z}{1+e^z}$ be the logistic (sigmoid) function.

Then, for observation $i$,

- .note[outcome] $Y_i \in \{0,1\}$, .grey-light[where 1 indicates *success*,]
- .note[dist] $Y_i \sim \text{Bernoulli}(p_i)$ .grey-light[with prob. of *success*] $\color{#cccccc}{p_i}$.grey-light[,]
- .note[prob] $p_i = \sigma(\beta_0 + \beta_1 x_{1,i} + \cdots + \beta_p x_{p,i}) = \sigma(x_i' \beta)$;

A single .attn[Bernoulli] experiment has .pink[probability mass function] (PMF)
$$
P(Y_i = y | X_i = x_i) = p_i^y (1-p_i)^{1-y} = \sigma(x_i' \beta)^y [1-\sigma(x_i' \beta)]^{1-y}
$$

---
## *Formally:* The likelihood function

.note[Define] The .attn[likelihood function] $\color{#e64173}{\mathcal{L}(\beta)}$ is the .purple[joint probability] of
<br>observing our data as a function of the parameters $(\beta)$.

.note[Recall] The .purple[joint probability] of independent events is
<br>the .slate.it[product] of their .pink[individual probabilities].

So the .attn[likelihood function] is the .slate[product] of individual probabilities, given $\beta$
\begin{align}
\color{#e64173}{\mathcal{L}(\beta)} &= \prod_{i=1}^n P(Y_i = y_i | X_i = x_i) 
\\
&= \prod_{i=1}^n \sigma(x_i' \beta)^{y_i} [1-\sigma(x_i' \beta)]^{1-y_i}
\end{align}

--
.note[Notice:] Different choices of $\beta$ lead to different .pink[likelihoods] (we want to max).

---
## *Formally:* The log likelihood function

From the .it[likelihood function]
$$
\mathcal{L}(\beta) = \prod_{i=1}^n \sigma(x_i' \beta)^{y_i} [1-\sigma(x_i' \beta)]^{1-y_i}
$$

we can get the .attn[log-likelihood function]

$$
\color{#e64173}{\ell(\beta)} = \sum_{i=1}^n \left( y_i \log[\sigma(x_i' \beta)] + (1-y_i) \log [1-\sigma(x_i' \beta)] \right)
$$

which is often nicer to work with and still achieves the same maximum.

---
## *Formally:* Maximium likelihood estimation

We .pink[estimate] logistic reg. by choosing the $\beta$s that .pink[max.] (log) .pink[likelihood]

\begin{align}
\ell(\beta)
&= \sum_{i=1}^n \left( y_i \log[\sigma(x_i' \beta)] + (1-y_i) \log [1-\sigma(x_i' \beta)] \right)
\\[.5em]
&= \sum_{i:y_i=1} \log[\sigma(x_i' \beta)] + \sum_{i:y_i=0} \log [1-\sigma(x_i' \beta)]
\\[.5em]
&= \sum_{i:y_i=1} \log[p(x_i)] + \sum_{i:y_i=0} \log [1-p(x_i)]
\end{align}

.note[Intuition] Max. (log) likelihood function by choosing $\beta$s that
- make $p(x_i)$ large for individuals with $y_i = 1$,
- make $p(x_i)$ small for individuals with $y_i = 0$.

---
## *Formally:* Finding the max

MLE wants to find the vector $\beta$ that maximizes $\ell(\beta)$.

.slab.pink[Idea 1] Search blindly over (infinite) parameter space.
--
<br>.slab.slate.it[Problem] Inefficient/infeasible.

--

.slab.pink[Idea 2] Use calculus. Set first-order conditions to zero and solve<br>(And check second-order conditions.)
--
<br>.slab.slate.it[Problem] No closed-form solution for logistic regression.

--

That said, partial derivatives will still be helpful...

\begin{align}
  \dfrac{\ell(\beta)}{\beta_j} = \sum_{i=1}^{n} \big[ y_i - \sigma(x_i' \beta) \big] x_{j,i}
\end{align}

---
## *Formally:* Numerical optimization

.pink.slab[Idea 1] + .pink.slab[Idea 2] = .slab.pink[Idea 3] Numerical optimization algorithms.
<br>

.attn[Gradient ascent] uses derivatives (gradients) to iteratively improve guesses.
- start with a naïve guess of parameters,
- evaluate the gradient at that guess, .grey-light[(*where is steepest ascent?*)]
- update your guess in the direction of the gradient. .grey-light[(*move up the hill*)]
- repeat. .grey-light[(*keep hiking until you reach the peak*)]

\begin{align}
\beta_j^{(\text{new})}
&= \beta_j^{(\text{old})} + \eta \dfrac{\partial \ell(\beta)}{\partial \beta_j}(\beta^{(\text{old})})
\\
\implies \beta_j^{(\text{new})} &= \beta_j^{(\text{old})} + \eta \sum_{i=1}^{n} \big[ y_i - \sigma(x_i' \beta^{(\text{old})}) \big] x_{j,i}
\end{align}
...
1. $\eta$ is the .attn[learning rate] (step size);<br>too big and you might overshoot, too small and it takes forever.

2. Gradient ascent/descent is one of many numerical optimization algorithms.<br>Others include .attn[Newton-Raphson], .attn[BFGS], and .attn[*Stochastic* gradient descent].

3. As with other estimators, you'll use .attn[existing software] in practice.

4. *Search algorithms* are centeral to many models, _e.g._, .attn[neural networks].
...
```{r, summary-glm, highlight.output = 5:8}
...
```{r, plot-default-logistic-2, echo = FALSE, cache = TRUE}
ggplot(data = default_df, aes(x = balance, y = i_default)) +
geom_point(alpha = 0.05, size = 3.5, color = purple) +
geom_line(stat = "smooth", color = red_pink, method = lm, size = 1.5, alpha = 0.2) +
geom_line(stat = "smooth", color = orange, method = "glm", method.args = list(family = "binomial"), size = 1.5) +
...
```{r, cm-right-wrong, echo = FALSE, eval = T}
conf_mat %>%
...
```{r, cm-sensitivity, echo = FALSE, eval = T}
conf_mat %>%
...
```{r, cm-specificity, echo = FALSE, eval = T}
conf_mat %>%
...
```{r, cm-precision, echo = FALSE, eval = T}
conf_mat %>%
...
.attn[Negative prediction value:]<br>the share of predicted negatives $(\hat{Y}=0)$ that are correct.

.center[
.b[NPV] = .hi-orange[TN] / (.hi-orange[TN] + .hi-purple[FN])
]

```{r, cm-npv, echo = FALSE, eval = T}
conf_mat %>%
set_text_color(3,2:4,purple) %>%
set_text_color(3,3,orange)
```

.note[Note:] NPV is not commonly used.
...
```{r, calc-threshold, include = FALSE, cache = TRUE}
...
```{r, calc-roc, include = FALSE, cache = TRUE}
...
```{r, plot-roc, echo = FALSE, fig.height = 6, cache = TRUE}
...
```{r, plot-auc, echo = FALSE, fig.height = 6, cache = TRUE}
ggplot(data = roc_df, aes(x = fpr, y = tpr)) +
geom_ribbon(
  aes(ymin = 0, ymax = tpr),
...
```{r, roc-ex1-threshold, echo = FALSE}
ggplot(
  data = tibble(x = seq(0, 1, 0.001)),
  aes(x = x)
) +
...
```{r, roc-ex1-threshold2, echo = FALSE}
ggplot(
  data = tibble(x = seq(0, 1, 0.001)),
  aes(x = x)
) +
